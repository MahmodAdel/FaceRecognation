...
import com.example.mahmoudfcih.simpleblogapp.helper.SampleApp;
...
public class AdminPersonGroupActivity extends AppCompatActivity {
    ...
    private void addPerson() {
        ...
        Intent intent = new Intent(this, AdminPersonActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_person_group);
        ...
        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (!personGridViewAdapter.longPressed) {
                    final String personId = personGridViewAdapter.personIdList.get(position);
                   /* FireBaseHelper.Algorithm algorithm=new FireBaseHelper.Algorithm();
                    algorithm.getPersonName(personId, personGroupId, new FireBaseHelper.OnGetDataListener() {
                        @Override
                        public void onSuccess(Object PersonName) {
                            Intent intent = new Intent(AdminPersonGroupActivity.this, AdminPersonActivity.class);
                    ...
                    String personName = StorageHelper.getPersonName(
                            personId, personGroupId, AdminPersonGroupActivity.this);
                    Intent intent = new Intent(AdminPersonGroupActivity.this, AdminPersonActivity.class);
                    intent.putExtra("AddNewPerson", false);
    ...
    private void doneAndSave(boolean trainPersonGroup) {
      ...
      //  FireBaseHelper.Algorithm algorithm=new FireBaseHelper.Algorithm();
     //  algorithm.setPersonGroupName(personGroupId,newPersonGroupName);
        StorageHelper.setPersonGroupName(personGroupId, newPersonGroupName, AdminPersonGroupActivity.this);
    ...
    private class PersonGridViewAdapter extends BaseAdapter {
        ...
        PersonGridViewAdapter() {
            ...
            final Set<String> personIdSet = StorageHelper.getAllPersonIds(personGroupId, AdminPersonGroupActivity.this);
            ...
            algorithm.getAllPersonIds(personGroupId, new FireBaseHelper.OnGetDataListener() {
                ...
                public void onSuccess(Object Data) {
                    ...
                    for (FireBaseHelper.Algorithm personId: persons) {
                        personIdList.add(personId.personId);
                        personChecked.add(false);
                    }
                }
            });*/
      ...
      //      FireBaseHelper.Algorithm algorithm1=new FireBaseHelper.Algorithm();
            final View finalConvertView1 = convertView;
            Set<String> faceIdSet = StorageHelper.getAllFaceIds(personId, AdminPersonGroupActivity.this);
            if (!faceIdSet.isEmpty()) {
                Iterator<String> it = faceIdSet.iterator();

                Uri uri = Uri.parse(StorageHelper.getFaceUri(it.next(), AdminPersonGroupActivity.this));
                ((ImageView) finalConvertView1.findViewById(R.id.image_person)).setImageURI(uri);
         ...
         /*   algorithm1.getAllFaceIds(personId, new FireBaseHelper.OnGetDataListener() {
                @Override
                public void onSuccess(Object Data) {
                    //  Set<String> faceIdSet = StorageHelper.getAllFaceIds(personId, AdminPersonGroupActivity.this);
                    Set<String> faceIdSet=new ArraySet<String>();
                    final List<FireBaseHelper.Algorithm> FacesId= (List<FireBaseHelper.Algorithm>) Data;
                    for (FireBaseHelper.Algorithm FaceId : FacesId) {
                        faceIdSet.add(FaceId.personFaceId);
                    }
                    if (!faceIdSet.isEmpty()) {
                     //   int i=0;
                        Iterator<String> it = faceIdSet.iterator();
                      ...
                      //  Uri uri = Uri.parse(StorageHelper.getFaceUri(it.next(), AdminPersonGroupActivity.this));
                        FireBaseHelper.Algorithm algorithm2=new FireBaseHelper.Algorithm();
                        algorithm2.getFaceUri(it.next(), new FireBaseHelper.OnGetDataListener() {
                            @Override
                            public void onSuccess(final Object FaceUri) {
                                final Uri uri=Uri.parse(FaceUri.toString());
                         //       ((ImageView) finalConvertView1.findViewById(R.id.image_person)).setImageURI(uri);
                                final ImageView loadedImage=(ImageView)finalConvertView1.findViewById(R.id.image_person);
                                Picasso.with(AdminPersonGroupActivity.this).load(uri).networkPolicy(NetworkPolicy.OFFLINE)
         ...
         //   FireBaseHelper.Algorithm algorithm=new FireBaseHelper.Algorithm();
           ...
           /* algorithm.getPersonName(personId, personGroupId, new FireBaseHelper.OnGetDataListener() {
                @Override
                public void onSuccess(Object PersonName) {
                    ((TextView) finalConvertView.findViewById(R.id.text_person)).setText(PersonName.toString());
                }
            });*/
            String personName = StorageHelper.getPersonName(personId, personGroupId, AdminPersonGroupActivity.this);

            ((TextView) finalConvertView.findViewById(R.id.text_person)).setText(personName);
